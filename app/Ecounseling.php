...
class Ecounseling extends Model
{
    public function getspefapp($date){
    	return DB::select("SELECT * FROM ecounselings WHERE counsel_date = ? AND psych_id = ? AND counsel_status = 'active' ORDER BY counsel_time ASC", $date);
    }
    ...
    public function getpatients($id){
        return DB::select("SELECT * FROM patients WHERE psych_id = $id");
    }
